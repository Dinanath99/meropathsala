...
//import InterestModal from "@/components/student/InterestModal";
import InterestModal from "./student/InterestModal";
...
const Login = () => {
  ...
  // New states to manage logged-in user and modal visibility
  ...
  useEffect(() => {
    ...
    if (loginIsSuccess && loginData) {
      ...
      // Check if modal should be shown for student with incomplete profile
      if (
        loginData.user.role === "student" &&
        (
          !loginData.user.skills?.length ||
          !loginData.user.interests?.length ||
          !loginData.user.experienceLevel
        )
      ) {
  ...
  return (
    <div className="flex items-center w-full justify-center mt-20">
      ...
      {/* InterestModal displayed after login for students with incomplete profile */}
      {showInterestModal && (
        <InterestModal
          user={loggedInUser}
          refetch={async () => {
