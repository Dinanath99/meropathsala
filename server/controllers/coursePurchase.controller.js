  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      if (user?.email) {
        const message = `
          Hi ${user.name},<br/><br/>
          Your payment for <strong>${purchase.courseId.courseTitle}</strong> was successful!<br/>
          Amount: NPR ${purchase.amount}<br/><br/>
          You can now access all lectures. Happy learning! 🚀
        `;
        await sendEmail({
          email: user.email,
